...
def preizkusi_podatke(xs: np.ndarray, y: np.ndarray):
    ...
    modeli = [
        ("Naključni gozd", RandomForestClassifier(random_state=1234, max_features="sqrt", n_estimators=100)),
...
#
########################################################################################################################
LOGGER = create_logger("", stopnja=logging.WARNING)
...
def pretvornik_ur(ura_kot_niz):
    ure, minute, _ = ura_kot_niz.split(":")
    return int(ure) + int(minute) / 60


def razsiri_z_hours(business_ids):
    tabela_hours = PODATKI["hours"]
    # 1)
    dnevi = tabela_hours["day"].unique()
    # 2)
    novi_stolpci = [f"{dan}_{dogodek}" for dan in dnevi for dogodek in ["open", "close"]]
    # 3) lahko bi namesto funkcije pretvornik_ur uporabil kar
    #    lambda x: int(x[:2])
    for stolpec in ["open time", "close time"]:
        tabela_hours[stolpec] = tabela_hours[stolpec].apply(pretvornik_ur)
    # 4) prednastavljena vrednost je nekaj,
    #    kar je izven [min, max] za vse stolpce
    nova_tabela = pd.DataFrame(
        index=business_ids,
        columns=novi_stolpci,
        data=30
    )
    # 5)
    for _, vrsta in tabela_hours.iterrows():
        biznis = vrsta["business_id"]
        dan = vrsta["day"]
        odprtje = vrsta["open time"]
        zaprtje = vrsta["close time"]
        nova_tabela.at[biznis, f"{dan}_open"] = odprtje
        nova_tabela.at[biznis, f"{dan}_close"] = zaprtje
    # 6) vsi stolpci so že numerični, zato samo pretvorimo v np.array
    return np.array(nova_tabela)
...
def razsiri_z_attributes(business_ids):
    tabela_attributes = PODATKI["attributes"]
    prednastavljena = "9999"  # smo "preverili", da je ok
    # NAMESTO TEGA:
    # # 1) & 2) (še list okoli unique)
    # novi_stolpci = list(tabela_attributes["name"].unique())
    # # 3)
    # nova_tabela = pd.DataFrame(
    #     index=business_ids,
    #     columns=novi_stolpci,
    #     data=prednastavljena
    # )
    # # 4)
    # for _, vrsta in tabela_attributes.iterrows():
    #     biznis = vrsta["business_id"]
    #     atribut = vrsta["name"]
    #     vrednost = vrsta["value"]
    #     nova_tabela.at[biznis, atribut] = vrednost
    # # 5)
    #
    # LAHKO NAREDIMO ENOSTAVNO (pivot nadomesti kodo zgoraj, a morda našteje vrstice v napačnem vrstem redu,
    #                           zato jih še premešamo z reindex)
    nova_tabela = tabela_attributes\
        .pivot(index='business_id', columns='name', values='value').fillna(prednastavljena)\
        .reindex(business_ids, fill_value=prednastavljena)
    # potem pa še preostanek
    for stolpec in nova_tabela.columns:
        numericne = pd.to_numeric(nova_tabela[stolpec], errors="coerce")
        if all(numericne.notnull()):
            nova_tabela[stolpec] = numericne
    # 6
    return pretvori_vse_one_hot(nova_tabela, nova_tabela.columns, drop=[prednastavljena])
...
for podseznam in generiraj_vse_neprazne_podsezname(n_blokov):
    uporabljena_imena = [imena_blokov[i] for i in podseznam]
    uporabljeni_bloki = [vsi_bloki[i] for i in podseznam]
    xs = np.block(uporabljeni_bloki)
    print(f"Rezultati za bloke {uporabljena_imena}:")
